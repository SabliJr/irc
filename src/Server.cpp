...
// }
void Server::commandRouter(Client *client, std::string cmd)
{
	std::cout << GREEN << "commandRouter called:" << cmd << RESET << std::endl;
	if (cmd.empty())
		return;

	std::string command = cmd;

	if (strncmp(command.c_str(), "PASS", 4) == 0)
	{
	...
	{
		std::string errorMsg = "421 " + client->getNickname() + " " + command + " :Unknown command\r\n";
