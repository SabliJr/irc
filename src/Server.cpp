...
void Server::handleWho(Client *client, const std::string &cmd)
{
	...
	if (!mask.empty() && mask[0] == '#')
	{
		...
		std::string end = ":ircserv 315 " + client->getNickname() + " " + mask + " :End of WHO list\r\n";
		sendNonBlockingCommand(client->getSocketFd(), end);
		return;
	}
...
{
	std::cout << BOLDYELLOW "[" << RESET BOLD "RECV" BOLDYELLOW "] " << RESET << std::endl;
	char buffer[1024];
	...
	while (1)
	{
		...
		std::cout << BOLDMAGENTA "[" << RESET BOLD << "CMDS" << BOLDMAGENTA "] " << RESET << command << std::endl;
