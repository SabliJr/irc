...
void Server::serverInit() {
  ...
  while (this->_signal == false) {
    int ret = poll(this->_pollFds.data(), this->_pollFds.size(),
                   -1); //! Explain poll what it does in the program
...
void Server::handleJoin(Client *client, const std::string &cmd) {
  std::cout << GREEN << "[LOGS]" << RESET << "handleJoin called: " << cmd
            << std::endl;
...
void Server::handlePrivmsg(Client *client, const std::string &cmd) {
  std::cout << GREEN << "[LOGS]" << RESET << "handlePrivmsg called: " << cmd
            << std::endl;
...
void Server::handlePart(Client *client, const std::string &cmd) {
  std::cout << GREEN << "[LOGS]" << RESET << "handlePart called: " << cmd
            << std::endl;
...
void Server::handleMode(Client *client, const std::string &cmd) {
  std::cout << GREEN << "[LOGS]" << RESET << "handleMode called: " << cmd
            << std::endl;
...
void Server::handleTopic(Client *client, const std::string &cmd) {
  std::cout << GREEN << "[LOGS]" << RESET << "handleTopic called: " << cmd
            << std::endl;
  ...
  // Fix: Find the channel name position in the command, then look for the topic
  // after it
...
void Server::handleInvite(Client *client, const std::string &cmd) {
  std::cout << GREEN << "[LOGS]" << RESET << "handleInvite called: " << cmd
            << std::endl;
...
void Server::handleKick(Client *client, const std::string &cmd) {
  std::cout << GREEN << "[LOGS]" << RESET << "handleKick called: " << cmd
            << std::endl;
...
void Server::handleChannelMode(Client *client, const std::string &cmd) {
  std::cout << GREEN << "[LOGS]" << RESET << "handleChannelMode called: " << cmd
            << std::endl;
...
void Server::handlePing(Client *client, const std::string &cmd) {
  std::cout << GREEN << "[LOGS]" << RESET << "handlePing called: " << cmd
            << std::endl;
  // Reply with PONG
  size_t pos = cmd.find(' ');
...
void Server::commandRouter(Client *client, const std::string &cmd) {
  std::cout << GREEN << "[LOGS]" << RESET << "commandRouter() called: " << cmd
            << std::endl;
  if (cmd.empty())
    return;
...
void Server::handleWho(Client *client, const std::string &cmd) {
  std::cout << GREEN << "[LOGS]" << RESET << "handleWho called: " << cmd
            << std::endl;
  std::istringstream iss(cmd);
...
void Server::handleMessage(int fd, Client *client) {
  std::cout << GREEN << "\n[LOGS]" << RESET << "handleMessage called"
            << std::endl;
  ...
  while (1) {
    ...
    if (!client->getAuthenticated()) {
      ...
      std::cout << RED << "Authenticated command: " << command << RESET
                << std::endl;
...
void Server::handleRegistrationMessage(Client &client, const std::string &cmd) {
  std::cout << GREEN << "[LOGS]" << RESET
            << "handleRegistrationMessage() called: " << cmd << std::endl;
...
void Server::handleCap(Client *client, const std::string &cmd) {
  std::cout << GREEN << "[LOGS]" << RESET << "handleCap called: " << cmd
            << std::endl;
  if (cmd == "CAP LS" || cmd.find("CAP LS") == 0) {
    ...
    std::string response = ":ircserv CAP * ACK :capabilities acknowledged\r\n";
    sendNonBlockingCommand(client->getSocketFd(), response);
  }
}

void Server::handlePass(Client *client, const std::string &cmd) {
  std::cout << GREEN << "[LOGS]" << RESET << "handlePass called: " << cmd
            << std::endl;
...
void Server::handleNick(Client *client, const std::string &cmd) {
  std::cout << GREEN << "[LOGS]" << RESET << "handleNick called: " << cmd
            << std::endl;
...
void Server::handleUser(Client *client, const std::string &cmd) {
  std::cout << GREEN << "[LOGS]" << RESET << "handleUser() called: " << cmd
            << std::endl;
...
void Server::acceptClient() {
  std::cout << GREEN << "[LOGS]" << RESET << "acceptClient() called"
            << std::endl;
...
void Server::closeSockets() {
  std::cout << GREEN << "[LOGS]" << RESET << "closeSockets() called"
            << std::endl;
  for (size_t i = 0; i < this->_pollFds.size(); i++) {
...
void Server::createSocket() {
  std::cout << GREEN << "[LOGS]" << RESET << "createSocket() called"
            << std::endl;
  int en = 1;
