...
Channel::Channel(std::string name) : _name(name), _password(""), _topic(""),  _clientCount(1), _maxUsers(10),
 _inviteOnly(false), _topicProtected(false), _clients(std::vector<Client *>()), _operators(std::vector<Client *>()) {}
...
bool Channel::isUserLimitEnabled() {
    return this->_userLimitEnabled;
}
...
void Channel::broadcast(const std::string &message, Client* exclude)
{
    for (size_t i = 0; i < this->_clients.size(); i++)
    {
        ...
        {
            ssize_t bytesSent = send(this->_clients[i]->getSocketFd(), message.c_str(), message.length(), 0);
            if (bytesSent == -1)
            {
